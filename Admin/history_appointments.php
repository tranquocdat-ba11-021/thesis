...
<body>
    <div class="wrapper">
        ...
        <div id="content2" class="container mt-4">
            ...
            if ($type == 2) {
                $total_appointments_sql .= " AND s.type = 2";
            } else if ($type == 1) {
                $total_appointments_sql .= " AND s.type = 1";
            }
            ...
            $sql_history = "SELECT a.id, u.full_name, p.pet_name, a.appointment_date, a.appointment_start_time, a.appointment_end_time, s.type_name, a.status
                            FROM appointments a
                            JOIN registered_users u ON a.user_id = u.id
                            JOIN user_pets p ON a.pet_id = p.id
                            JOIN services s ON a.service = s.id_service
            ...
            if ($type == 2) {
                $sql_history .= " AND s.type = 2";
            } else if ($type == 1) {
                $sql_history .= " AND s.type = 1";
            }
                    ...
                    while ($row = $result->fetch_assoc()) {
                        echo "
                            <tr>
                                ...
                                <td>{$row['status']}</td>
                                <td>
                                    <a class='btn btn-primary btn-sm' href='edit_booking.php?id={$row['id']}'>Edit</a>
                                </td>
                            </tr>
                        ";
                    }
